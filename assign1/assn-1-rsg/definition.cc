...
Definition::Definition(ifstream& infile)
{
  ...
  // peek returns next character in input sequence without extracting it
  while (infile.peek() != '}') {
    Production possibleExpansion(infile); // making new object of Production
    possibleExpansions.push_back (possibleExpansion); // defined in definition.h
  }
