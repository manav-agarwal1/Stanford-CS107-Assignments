...
class Definition {
 ...
 public:
  ...
  /**
   * Method: getAllProduction
   * ---------------------------
   * Returns all productions
   * Made for testing purposes
   * First const signifies returned vector
   * cannot be modified. Second const means
   * that this is passed as pointer to variables of
   * Definition and those cannot be modified within
   * function body.
   * @return return immutable reference to all productions stored
   * 
   */
  const vector<Production>& getAllProductions() const { return possibleExpansions; };
